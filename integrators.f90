subroutine compute_forces(F, phi_minus, phi_plus, N, x)
  implicit none
  integer :: i
  integer, intent(in) :: N
...
subroutine Velocity_Verlet_steps(steps, v, x, phi_minus, phi_plus, N, dt)
  implicit none
  integer :: steps, i, N
  real(8) :: F(0:N), v(0:N), x(0:N), phi_minus(0:N), phi_plus(0:N), dt

  call compute_forces(F, phi_minus, phi_plus, N, x)
  v = v + F*0.5d0*dt
  do i=1,steps
    x = x + v*dt
    call compute_forces(F, phi_minus, phi_plus, N, x)
...
subroutine PEFRL_steps(steps, v, x, phi_minus, phi_plus, N, dt)
